...
export function binaryToDecimal(binaryArray) 
{
  let decimalValue = 0;
...
export function fixedHammingCode(hammingBlock, positionOfError) 
{
...
export function printMatrix(array) {
  ...
  console.log(result); // Log the matrix to the browser console
}


export function zeroCheck(hammingBlock) 
{
...
export function parityCheck(hammingBlock) 
{
...
export function parityCheckForOriginalBlock(hammingBlock) {
  let answersToParityChecks = [];
  let totalOnes = 0;
  for (let i = 0; i < 16; ++i) {
    totalOnes += hammingBlock[i];
...
export function generateRandomHammingBlock() {
...
export function introduceRandomError(originalBlock) {
  const errorIndex = Math.floor(Math.random() * originalBlock.length);
  console.log("Original Block:", originalBlock); // Debugging statement
  console.log("Error Index:", errorIndex); // Debugging statement
  
  const incorrectBlock = [...originalBlock];
  incorrectBlock[errorIndex] = incorrectBlock[errorIndex] === 0 ? 1 : 0; // Toggle the bit

  console.log("Incorrect Block:", incorrectBlock); // Debugging statement
  return incorrectBlock;
}


export function correctCode(errorCode) 
{
  let correctedCode = parityCheck(errorCode);
  correctedCode = zeroCheck(correctedCode);
  console.log();
...
const errorCode = introduceRandomError(hammingBlock); 
...
const fixedCode = correctCode(errorCode);
console.log("Your fixed block of code is:");
printMatrix(fixedCode);
console.log();
